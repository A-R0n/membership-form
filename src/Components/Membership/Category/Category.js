...
export default class Category extends Component {
	constructor() {
		...
		this.state = {
			type: '',
			classification: null,
			prices: [ { junior_walk: 800 }, { junior_cart: 1200 }, { junior_range: 400 },
				{ junior_gpa: 1400 },{ adult_walk: 2600 }, { adult_cart: 3200 },
				{ adult_range: 400 },{ adult_gpa: 675 },{ family_walk: 3250 },
				{ family_cart: 4050 },{ family_range: 550 }, { family_gpa: 825 } ]
		};
	}
	...
	async showPrice(a){
		let obj = this.state.prices.find(o => {return o[`${a}`]})
		const num = d3.select('#price').append('text')
		.text('$' + obj[`${a}`]).attr('y', 20).attr('x', 10).style('fill', 'green');
		await this.rectSize()
	};
	...
	render() {
		...
		return (
			...
			<svg  className='MembershipTitleField' height={70}>
					<text x='100' y='50' className='MembershipTitle'>Select Category</text>
					</svg>
				...
				<p className="Options">
					Options:
					<select name="membership" onChange={(e) => this.handleOptions(e.target.value)}>
				...
				<div className="totalCost">
					Total:{' '}
						...
						<g>
							<text id='easyMoney' stoke='black' x="10" y="20">$0 </text>
